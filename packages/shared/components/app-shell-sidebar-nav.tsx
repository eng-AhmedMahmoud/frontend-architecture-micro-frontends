...
interface AppShellSidebarNavProps {
  isExternalNav?: (to: string) => boolean;
  isPathActive?: (pathname: string, to: string) => boolean;
  profileExternalNavigation?: boolean;
}

function defaultPathActive(pathname: string, to: string) {
  return pathname === to || (to !== "/" && pathname.startsWith(to));
}

export function AppShellSidebarNav({
  isExternalNav = () => false,
  isPathActive = defaultPathActive,
  profileExternalNavigation = false,
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <nav className="mt-4 flex-1 space-y-1">
        {appShellNavItems.filter((item) => hasPermission(item.permission)).map((item) => {
